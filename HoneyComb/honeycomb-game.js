...
(function (G) {
  ...
  G.startLevelTimer = function () {
    ...
    G.timerIntervalId = setInterval(function () {
      ...
      if (elapsed >= G.levelTimeLimitSec) {
        // 0초 동시 달성(미션 성공)이면 성공을 우선한다.
        // pendingLevelUp/levelUpUntil 상태에서도 게임오버 오버레이가 뜨지 않게 방지.
        var need = G.getDiamondsToNextLevel(G.level);
        ...
        if (G.pendingLevelUp || G.levelUpUntil > 0 || missionDone) {
          if (missionDone && !G.pendingLevelUp) {
            ...
            G.missionCompleteText = '미션 완료!';
            G.missionCompleteUntil = Date.now() + 900;
          }
          return;
        }
        if (G.timerIntervalId) clearInterval(G.timerIntervalId);
  ...
  G.applyRemove = function (toRemove, fromSwap) {
    ...
    setTimeout(function () {
      ...
      if (G.diamondsRemovedThisLevel >= needToNext) {
        G.pendingLevelUp = true;
        if (!G.missionCompleteUntil || Date.now() > G.missionCompleteUntil) {
          G.missionCompleteText = '미션 완료!';
          G.missionCompleteUntil = Date.now() + 900;
        }
        G.comboDrawLoop();
  ...
  G.comboDrawLoop = function () {
    ...
    if (G.pendingLevelUp) {
      ...
      var missionMsgDone = now >= (G.missionCompleteUntil || 0);
      if (comboDone) {
        if (!missionMsgDone) {
        ...
        if (G.timerIntervalId) clearInterval(G.timerIntervalId);
        G.timerIntervalId = null;
        // 레벨업이 시작되면 게임오버 오버레이가 남지 않도록 강제 정리
        G.gameOver = false;
