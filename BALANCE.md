...
| 항목 | 파일 | 함수/상수 | 설명 |
|------|------|-----------|------|
| 레벨별 행/열 | honeycomb-grid.js | `getGridSize(lv)` | Lv 1~10 완만, 11~20 확대, 21~100 최대 12×12 |
| 모바일 캡 | honeycomb-grid.js | `applyGridSize()` 내 `maxRC` | CANVAS_WIDTH < 380 → 최대 9×9 |

- **목표**: Lv 10 이하는 블럭 수 과다 방지(7×7=49 이하), 고레벨에서만 12×12까지 확대.
